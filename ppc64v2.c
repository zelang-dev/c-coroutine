...
static thread_local struct ppc64_context* co_current_handle = 0;
...
co_routine_t *co_active(void) {
  if(!co_active_handle) {
    co_active_handle = (struct ppc64_context*)CO_MALLOC(MIN_STACK + sizeof(struct ppc64_context));
  }
  return (co_routine_t *)co_active_handle;
}
...
co_routine_t *co_derive(void* memory, unsigned int size, co_callable_t func, void *args) {
  ...
  *(uint64_t*)sp = (uint64_t)(sp + MIN_STACK_FRAME);
  ...
  context->gprs[12] = (uint64_t)func;
  context->lr       = (uint64_t)func;

  return (co_routine_t *)memory;
}
