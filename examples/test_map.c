...
int test_map_push_pop() {
    ...
    value = has_const_char_64(map_pop(list));
    printf("--> %s\n", value);
...
int test_map_shift_unshift() {
    ...
    ASSERT_XEQ(map_count(list), 1);
    value = map_unshift(list)->value.str;
    ASSERT_PTR(value, _value_1);
    ASSERT_XEQ(map_count(list), 0);
...
static int _verify_map_values(map_t *list, int count, ...) {
    ...
    for (i = 0; i < count; i++) {
        ...
        ASSERT_STR((char *)values[ i ], has(iterator).str);
...
int test_map_get_reverse() {
    ...
    list = map(NULL, 3, kv("__1_", &index1), kv("__2_", &index2), kv("__3_", &index3));
    ASSERT_NOTNULL(list);
...
int test_array_iterator() {
    ...
    iterator = iter_new(list, true);
    ASSERT_NOTNULL(iterator);
    ASSERT_PTR(has(iterator).str, _value_1);
    iterator = iter_next(iterator);
    ASSERT_NOTNULL(iterator);
    ASSERT_PTR(has(iterator).str, _value_2);
    iterator = iter_next(iterator);
    ASSERT_NOTNULL(iterator);
    ASSERT_PTR(has(iterator).str, _value_3);
    iterator = iter_next(iterator);
    ASSERT_NULL(iterator);
...
int test_array_slice_foreach() {
    ...
    list = array(NULL, 4, "John", "Paul", "George", "Ringo");
    part = slice(list, 0, 2);
    ASSERT_NOTNULL(part);
    i = 0;
    foreach(item in part) {
        printf("item key is %s, and value is %s\n", indic(item), has(item).str);
        ASSERT_STR(indic(item), co_itoa(i));
        i++;
    }
    map_free(list);
