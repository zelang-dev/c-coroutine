...
void_t worker_client(params_t args) {
    ...
    ASSERT_WORKER(is_str_eq("pipe_listen", args[1].char_ptr));
...
TEST(pipe_listen) {
    ...
    rid_t res = go(worker_client, 3, 1000, "pipe_listen", "pipe_handler");
    ...
    ASSERT_STR(result_for(res).char_ptr, "pipe_handler");
