...
void_t worker_client(params_t args) {
    ...
    ASSERT_WORKER(is_str_eq("stream_listen", args[1].char_ptr));
...
TEST(stream_listen) {
    ...
    rid_t res = go(worker_client, 3, 1000, "stream_listen", "stream_handler");

    ASSERT_TRUE(is_tcp(socket = stream_bind("http://127.0.0.1:8090", 0)));
    ASSERT_FALSE(is_tls(socket));

    ASSERT_TRUE(is_tcp(client = stream_listen(socket, 1024)));
    ...
    ASSERT_STR(result_for(res).char_ptr, "stream_handler");
