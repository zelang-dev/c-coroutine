...
#define CO_PPC64 (_ARCH_PPC64 || __PPC64__ || __ppc64__ || __powerpc64__)

/* whether function calls are indirect through a descriptor, or are directly to function */
#ifndef CO_PPCDESC
  #if !_CALL_SYSV && (_CALL_AIX || _CALL_AIXDESC || (CO_PPC64 && (!defined(_CALL_ELF) || _CALL_ELF == 1)))
    #define CO_PPCDESC 1
  #endif
#endif
  ...
  section(text)
#endif
static const uint32_t co_ppc_code[1024] = {
  #if CO_PPC64
  0x7d000026,  /* mfcr    r8          */
  ...
  #define CO_SWAP_ASM(x, y) ((void (*)(co_routine_t *, co_routine_t *))(uintptr_t)x)(x, y)
#else
  /* function call goes directly to code */
  #define CO_SWAP_ASM(x, y) ((void (*)(co_routine_t *, co_routine_t *))(uintptr_t)co_ppc_code)(x, y)
#endif
...
static uint32_t* co_derive_(void* memory, unsigned size, uintptr_t entry) {
  ...
  if(t) {
    ...
    *(const void**)t = co_ppc_code;  /* set function pointer to swap routine */
  }
  #endif
...
co_routine_t *co_derive(void *memory, unsigned int size, co_callable_t entry_, void *args)
{
  ...
  return (co_routine_t *)t;
}

static uint32_t* co_create_(unsigned size, uintptr_t entry) {
  (void)entry;

  uint32_t* t = (uint32_t*)CO_MALLOC(size);
...
co_routine_t *co_create(unsigned int size, co_callable_t entry_)
{
  uintptr_t entry = (uintptr_t)entry_;
  uint32_t* t = 0;
...
static void co_init_(void) {
  ...
  if(page_size > 0) {
    ...
    uintptr_t begin = (uintptr_t)co_ppc_code;
    uintptr_t end   = begin + sizeof co_ppc_code;
